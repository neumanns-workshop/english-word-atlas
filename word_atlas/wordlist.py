...
class WordlistBuilder:
    ...
    def __init__(self, atlas: Optional[WordAtlas] = None, data_dir: Optional[Union[str, Path]] = None):
    ...
    def add_by_frequency(self, min_freq: float = 0, max_freq: Optional[float] = None) -> int:
        ...
        added_count = len(self.words) - original_count
        if added_count > 0:
            freq_desc = f">= {min_freq}" if max_freq is None else f"{min_freq}-{max_freq}"
            self.metadata["criteria"].append({
                "type": "frequency",
                "min_freq": min_freq,
                "max_freq": max_freq,
    ...
    def add_by_custom_filter(self, filter_func: Callable[[str, Dict[str, Any]], bool], 
                            description: str) -> int:
    ...
    def set_metadata(self, name: str = None, description: str = None, 
                    creator: str = None, tags: List[str] = None) -> None:
    ...
    @classmethod
    def load(cls, filename: Union[str, Path], atlas: Optional[WordAtlas] = None,
             data_dir: Optional[Union[str, Path]] = None) -> 'WordlistBuilder':
    ...
    def analyze(self) -> Dict[str, Any]:
        ...
        for attr in self.atlas.word_data.get(next(iter(self.atlas.word_data)), {}):
            if (attr.startswith("GSL_") or 
                attr.startswith("SWADESH_") or 
                attr.startswith("NGSL_") or 
                attr.startswith("OGDEN_") or
                attr == "GSL" or
                attr == "SWADESH" or
                attr == "NGSL" or
                attr == "OGDEN"):
        ...
        stats["wordlist_coverage"] = {
            attr: {
                "count": count,
                "percentage": round(count / len(self.words) * 100, 1) if self.words else 0
    ...
    def export_text(self, filename: Union[str, Path], include_metadata: bool = True) -> None:
