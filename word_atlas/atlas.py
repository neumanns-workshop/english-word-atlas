...
class WordAtlas:
    ...
    def _discover_sources(self) -> Dict[str, Path]:
        ...
        if not (self.sources_dir.exists() and self.sources_dir.is_dir()):
            print(f"Warning: Sources directory not found or is not a directory: {self.sources_dir}", file=sys.stderr)
            return sources
                ...
                with open(file_path, "r", encoding='utf-8') as f:
                    if file_path.suffix == ".json":
                        ...
                        if isinstance(source_data, list):
                            ...
                            print(f"Warning: Source JSON '{source_name}' ignored (not a JSON list).", file=sys.stderr)
                        ...
                        print(f"Warning: Unknown source file type for '{source_name}': {file_path.suffix}", file=sys.stderr)
                        continue

            except FileNotFoundError:
                 print(f"Warning: Source file '{source_name}' disappeared before loading: {file_path}", file=sys.stderr)
            except json.JSONDecodeError as e:
                print(f"Warning: Could not parse source JSON '{source_name}' from {file_path}: {e}", file=sys.stderr)
            except Exception as e: # Catch other potential read errors
                 print(f"Warning: Could not load source '{source_name}' from {file_path}: {e}", file=sys.stderr)

            # Report warnings after processing file
            if invalid_items:
                 print(f"Warning: Source '{source_name}' contained {invalid_items} non-string items.", file=sys.stderr)
            if unknown_words:
                 print(f"Warning: Source '{source_name}' contains {len(unknown_words)} words not found in master index (e.g., '{next(iter(unknown_words))}'). These will be ignored.", file=sys.stderr)
    ...
    def get_words_in_source(self, source_name: str) -> Set[str]:
        ...
        if source_name not in self._source_lists:
            if source_name not in self.available_sources:
                ...
                print(f"Warning: Returning empty set for source '{source_name}' which failed to load or was invalid.", file=sys.stderr)
                return set()
    ...
    def filter(
        ...
        if sources:
            ...
            if not filtered_words: return set() # Early exit if no words match sources
