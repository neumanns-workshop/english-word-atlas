...
class TestWordlistCommand:
    ...
    def test_wordlist_merge(
        self, mock_cli_atlas, mock_wordlist_builder, capsys, tmp_path
    ):
        ...
        with patch("word_atlas.cli.WordAtlas", return_value=mock_cli_atlas), patch(
            ...
            side_effect=[input1_builder, input2_builder],
        ) as mock_load:
            ...
            def mock_returned_set_metadata(
                name=None, description=None, creator=None, tags=None
            ):
                if name is not None:
                    mock_returned_builder.metadata["name"] = name
                if description is not None:
                    mock_returned_builder.metadata["description"] = description
                if creator is not None:
                    mock_returned_builder.metadata["creator"] = creator
                if tags is not None:
                    mock_returned_builder.metadata["tags"] = (
                        tags  # Check for None, not truthiness
                    )
        ...
        mock_load.assert_has_calls(
            [call("list1.json", mock_cli_atlas), call("list2.json", mock_cli_atlas)],
            any_order=False,
        )
