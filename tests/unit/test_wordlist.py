...
class TestWordlistBuilder:
    ...
    def test_init_invalid_atlas_type(self):
        ...
        assert "Provided atlas object does not have the expected methods" in str(excinfo.value)
    ...
    def test_add_by_source(self, mock_wordlist_builder):
        ...
        assert builder.words == {"apple", "banana"}
        # Verify the added words are indeed in GSL
        assert all("GSL" in mock_wordlist_builder.atlas.get_sources(word) for word in builder.words)
    ...
    def test_export_text_formatting_options(self, tmp_path, mock_data_dir):
        ...
        assert content_sort == ["kiwi", "apple", "banana"] # kiwi(4), apple(5), banana(6)

        # Test formatting to uppercase
        builder.export_text(filepath_format, word_format=str.upper, include_metadata=False)
        ...
        # Test both
        builder.export_text(filepath_both, sort_key=len, word_format=str.upper, include_metadata=False)
